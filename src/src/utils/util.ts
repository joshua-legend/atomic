...
export const initializeTodoList = ({ setTodoitems, setId }: initializeTodoListParams) => {
  const mappedArray = JSON.parse(localGet()).map((v: any, i: number) => ({ id: i, contents: v.contents }));
